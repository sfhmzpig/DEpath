...
void LoadCase(int no){
    std::string s = std::string{"case"} + std::to_string(no);
    std::ifstream is{s};
    std::istream_iterator<int> io{is};
    std::cout << s << std::endl;
    ...
    for (int i = 0; i != T; ++i){
        ...
        threat_point[i][1] = *io++;
        threat_point[i][2] = *io++;
        std::cout << threat_point[i][2] << std::endl;
...
void ZoomMap(){
    ...
    Y = Zoom(Y);
    Z = Zoom(Z);
    start_point[0] = Zoom(start_point[0]);
    start_point[1] = Zoom(start_point[1]);
    start_point[2] = Zoom(start_point[2]);
    end_point[0] = Zoom(end_point[0]);
    end_point[1] = Zoom(end_point[1]);
    end_point[2] = Zoom(end_point[2]);
    for(int i = 0; i != T; ++i){
        threat_point[i][0] = ZoomThre(threat_point[i][0]);
        threat_point[i][1] = ZoomThre(threat_point[i][1]);
        threat_point[i][2] = ZoomThre(threat_point[i][2]);
    }
    D = end_point[0] - start_point[0] + 1;
}
...
int ** Revoke(int **pop){
    for (int i = 1; i != D-1; ++i){
        for (int j = 0; j != 3; ++j){
            pop[i][j] = reZoom(pop[i][j]);
        }
    }
    for (int i = 0; i != 3 ; ++i){
        pop[0][i] = reZoom(pop[0][i]);
        pop[D-1][i] = reZoom(pop[D-1][i]);
    }
    return pop;
}
...
double SetCR(double ave_fit,double min_fit,double max_fit,double me_fit){
    double CR;
    if (ave_fit - me_fit >= precession){ //revise1
        CR = CRl + (CRu - CRl)*(max_fit - me_fit)/(max_fit - min_fit);
    } else {
        CR = CRl;
    }
    return CR;
...
int *** Selection(int *** popu_now, int *** popu_cross, int signal1, int iteTh){ //选择函数，从原来种群和交叉种群的每个个体中选择表现更好的个体作为新个体
    ...
    double now_min = minNum,cross_min = minNum;
    /*
    for (int i = 0; i != Pop; ++i){
        for (int j = 0; j != D; ++j){
    ...
    for (int i = 0; i != Pop; ++i){
        ...
        if (now_min - now_fit >= precession) //modify1
            now_min = now_fit;
        if (cross_min - cross_fit >= precession)
            cross_min = cross_fit;
...
//    std::cout << now_min << '\t' << cross_min << std::endl;
...
int ** GetOne(int *** popu_now, int iteTh){ //从种群中选择表现最好的个体
    ...
    for (int i = 0; i!= Pop; ++i){ //modify1
        if (pop_fits[i] - minFit <= precession){
            minFit = pop_fits[i];
            pop_index = i;
        }
    }
...
//    std::cout << Collosion(pop_now) << '\t' << NorDis(pop_now) << '\t' << Throught(pop_now) << std::endl;
    double w;
    w = 100 * iteTh/maxIte + 1;
    return (NorDis(pop_now) + Throught(pop_now));
...
std::array<double,Pop> PopFits(int ***popu_now, int iteTh){ //种群适应度
    ...
    w = 100 * iteTh/maxIte + 1;
    for (int i = 0; i != Pop; ++i){
        popu_fit[i] = NorDis(popu_now[i]) + Throught(popu_now[i]);
        //popu_fit[i] = NorDis(popu_now[i]);
    }
    return popu_fit;
};

int IsThrought(int x1, int y1, int z1, int x2, int y2, int z2){
    double k,b;
    double A,B,C;
    int x,y;
    double tmp_dis;
    ...
    C = b;
    for (int i = 0; i != T; ++i){
        tmp_dis = fabs((A * threat_point[i][0] + B * threat_point[i][1] + C)) / (sqrt(A*A + B*B));
        if (tmp_dis - threat_point[i][2] <= precession)
            return -1;
    }
    return 0;
}

double Throught(int **pop){
    int thr_num = 0;
    int x,y,z,x1,y1,z1;
    for (int i= 0; i != D-1; ++i){
        x = pop[i][0];
        ...
        y1 = pop[i+1][1];
        z1 = pop[i+1][2];
        if (IsThrought(x,y,z,x1,y1,z1) == -1)
            ++thr_num;
    }
//    std::cout << thr_num << std::endl;
    return (((double)thr_num) / (D-1));
}
...
int main() {
    ...
    auto re_zoom_get_one = Revoke(get_one);
